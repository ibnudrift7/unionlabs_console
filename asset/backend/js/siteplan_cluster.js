jQuery(document).ready(function ($) {
    ...
    function createPoly(element) {
        ...
        let poly = new Konva.Circle({
            x: points[0][0], // X position of bullet
            y: points[0][1], // Y position of bullet
            radius: 13,      // Adjust the size of the bullet
            fill: '#f00',
            stroke: 'green',
            strokeWidth: 2,
            opacity: 0.8,
        ...
        poly.on('click tap', function (evt) {
            console.log('konva_view_popup: ', konva_view_popup);
            if (konva_view_popup) {
                console.log('click Events', element);
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        ...
                        // konva-sites-detail
                        $('#konva-sites-details').html(`
                            <div class="site-info">
                                <h5 class="px-2">${res?.nama}</h5>
                                <table class="table table-sm table-bordered">
                                    <tr>
                                        <td>Cluster</td>
                                        <td>: ${res?.cluster_name}</td>
                                    </tr>
                                    <tr>
                                        <td>Project</td>
                                    ...
                                    <tr>
                                        <td>Status</td>
                                        <td>: ${res?.book_status ? 'Booked' : 'Available'}</td>
                                    </tr>
                                </table>
                            </div>
                        `);
        ...
        poly.on('mouseenter', function (evt) {
            let nama_blok = new Konva.Text({
                text: `${element.blok_name} - ${element.cluster_name}`,
                fontSize: 19,
                fontFamily: 'Calibri',
                fill: 'black',
                padding: 5
            ...
            let background = new Konva.Rect({
                width: nama_blok.width() + 20,
                height: nama_blok.height() + 10,
                fill: 'white',
                stroke: 'black',
                cornerRadius: 5
            });

            groupBubble.destroyChildren();
            groupBubble.add(background, nama_blok);
            ...
            const polyRect = poly.getClientRect({ relativeTo: stage }); // Ambil posisi relatif ke stage
            // console.log('Polygon Position:', polyRect);
            const ax = polyRect.x + polyRect.width / 2 - (background.width() / 2);
            const ay = polyRect.y - background.height();

            groupBubble.absolutePosition({
                x: ax,
                y: ay
            });

            groupBubble.moveToTop(); // Pastikan tooltip muncul di atas
            groupBubble.show();
            // layer.draw();
        })
    ...
    if (konva_set_points) {
        ...
        layerBackground.on('click', function (evt) {
            if (!evt.evt.altKey) {
                ...
                if (clickPoints.length >= 1 && evt.evt.shiftKey) {
        ...
        function load_select() {
            var urltestsn = $('#blok_id').attr('load-select');
            $.ajax({
                url: urltestsn,
            }).done(function (msg) {
                if ($('#blok_id').hasClass("select2-hidden-accessible")) {
                    $('#blok_id').select2('destroy');
                }
                let str = '<option value="">Pilih Blok</option>';
                for (let index = 0; index < msg.length; index++) {
                    const el = msg[index];
                    let cluster_nm = el.kode_blok + ' , ' + el.nama_cluster;
