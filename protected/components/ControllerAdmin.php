...
class ControllerAdmin extends CController
{
	...
	public $assetAdminBaseurl2;
	public $assetAdminUrl;
	...
	public function beforeAction($action)
	{
		...
		$this->assetAdminBaseurl2 = Yii::app()->baseUrl . '/asset/backend/images/';
		$this->assetAdminUrl = Yii::app()->baseUrl . '/asset/backend/assets_new';

		$this->pageTitle = $this->setting['title'];
		$this->metaDesc = $this->setting['description'];
		$this->metaKey = $this->setting['keywords'];

		$provincesData = City::model()->findAll(array('select' => 'province_id, province', 'group' => 'province_id'));
		$this->province_list = CHtml::listData($provincesData, 'province_id', 'province');

		$this->globalData = array(
			// 'listSpkCat' => $listSpkCat,
		);
