...
class Common
{
    ...
    public static function mail($config = array())
    {
        ...
        $config['from'] = ($config['from'] == '') ? 'from@mail.com' : $config['from'];
        $config['bcc'] = (empty($config['bcc'])) ? array('mail@gmail.com') : $config['bcc'];
    ...
    public static function mailPhpMailer($to = array(), $from = '', $subject = '', $message = '', $cc = array(), $bcc = array())
    {
        ...
        $mail->SetFrom($from, 'Utara - NoReply');
        $mail->Subject = $subject;
    ...
    public static function changeRupiahFormat($number)
    {
        // $value = str_replace('.00', '', $value);
        // $value = (int) $value;
        if ($number >= 1000000000000) {
            return 'Rp. ' . number_format($number / 1000000000000) . ' Triliun';
        } elseif ($number >= 1000000000) {
            return 'Rp. ' . number_format($number / 1000000000) . ' Miliar';
        } elseif ($number >= 1000000) {
            return 'Rp. ' . number_format($number / 1000000) . ' Juta';
        } else {
            return 'Rp. ' . number_format($number);
        }
    }

    public static function changeRupiah2($number)
    {
        return 'Rp. ' . number_format($number, 0, ',', '.');
    }

    public static function ShowDocImage($field, $folder)
    {
        $finfo = finfo_open(FILEINFO_MIME_TYPE);
        $mimeType = finfo_file($finfo, Yii::app()->basePath . '/../images/' . $folder . '/' . $field);
        finfo_close($finfo);
        if ($mimeType == 'application/pdf' || $mimeType == 'application/msword' || $mimeType == 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
            echo '<a href="' . Yii::app()->baseUrl . '/images/' . $folder . '/' . $field . '" target="_blank" class="btn btn-info btn-sm px-3">View Doc</a>';
        } else {
            $strs = '<a target="_blank" href="' . Yii::app()->baseUrl . '/images/' . $folder . '/' . $field . '">';
            $strs .='<img style="width: 100%; max-width: 110px;" src="' . Yii::app()->baseUrl . ImageHelper::thumb(1440, 683, '/images/' . $folder . '/' . $field, array('method' => 'resize', 'quality' => '90')) . '" />';
            $strs .= '</a>';
            echo $strs;
        }
    }
